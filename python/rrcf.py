...
class RCTree:
    ...
    def __repr__(self):
        ...
        def print_tree(node):
            if isinstance(node, Leaf):
                self.treestr += "LEAF {} ".format(node.n)
                self.treestr += '({})\n'.format(node.i)
            elif isinstance(node, RCTree):
                self.treestr += "RCTREE"
                self.treestr += '{0}{1}\n'.format("  ", '+')
                self.treestr += '{0} {1}{2}{2}'.format(self.depth, "|", "-")
                print_push(" ")
                print_tree(node.l)
                print_pop()
                self.treestr += '{0} {1}{2}{2}'.format(self.depth, '|', '-')
                ...
                self.treestr += "BRANCH q (dimension)={0} p (cut value)={1} n (leaf count)={2}".format(node.q, node.p, node.n)
                self.treestr += '{0}{1}\n'.format("  ", '+')
                self.treestr += '{0} {1}{2}{2}'.format(self.depth, "|", "-")
                print_push(" ")
                print_tree(node.l)
    ...
    def _cut(self, X, S, parent=None, side='l'):
        ...
        print("mktree before S2 %t", self.root, parent, side, child)
    ...
    def _mktree(self, X, S, N, I, parent=None, side='root', depth=0):
        ...
        depth -= 1
        if depth == 0:
            print("mktree %t", self.root, depth)
